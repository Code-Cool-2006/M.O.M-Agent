...
// Python scripts live next to main.js in development, in resources when packaged
function pyScript(name) {
  if (app.isPackaged) {
    return path.join(process.resourcesPath, 'backend', name);
  }
  return path.join(app.getAppPath(), 'backend', name);
}
...
import sys; sys.path.insert(0, ${JSON.stringify(path.join(app.isPackaged ? process.resourcesPath : app.getAppPath(), 'backend'))})
...
open(${JSON.stringify(path.join(state.session_dir, 'transcript.txt'))}, 'w').write(t)
  ...
  await runPython(pythonPath, ['-c', `
import sys, json; sys.path.insert(0, ${JSON.stringify(path.join(app.isPackaged ? process.resourcesPath : app.getAppPath(), 'backend'))})
from transcribe import transcribe
